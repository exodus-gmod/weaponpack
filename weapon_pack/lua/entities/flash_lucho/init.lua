...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/weapons/darky_m/rust/gl_ammo.mdl")
...
function ENT:EntityFacingFactor( theirent )
	...
	return (facingdir:Dot(dir)+2.4)/2
end
...
function ENT:Explosion()
	...
	for k,v in pairs(player.GetAll()) do
		...
		if !traceres.Hit or traceres.Fraction>=1 or traceres.Fraction<=0 then
			...
			if v:GetNWFloat("FlashDistance",v:GetShootPos():Distance(self:GetPos())) < 800 and v:GetNWFloat("FlashFactor",self:EntityFacingFactor(v)) < tr.endpos:Distance(self:GetPos(v)) then
				if v:GetNWFloat("FlashDistance",v:GetShootPos():Distance(self:GetPos())) < 1200 then
					v:SetDSP( 40 , false )
				elseif v:GetNWFloat("FlashDistance",v:GetShootPos():Distance(self:GetPos())) < 1000 then
					v:SetDSP( 38 , false )
