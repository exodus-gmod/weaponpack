...
SWEP.Purpose				= "A modular and balanced assault rifle build by remington. Its special magazines allow for 40 rounds." --Purpose Tooltip
SWEP.Instructions				= "" --Instructions Tooltip
SWEP.Spawnable				= true --Can you, as a normal user, spawn this?
SWEP.AdminSpawnable			= true --Can an adminstrator spawn this?  Does not tie into your admin mod necessarily, unless its coded to allow for GMod's default ranks somewhere in its code.  Evolve and ULX should work, but try to use weapon restriction rather than these.
SWEP.DrawCrosshair			= true		-- Draw the crosshair?
SWEP.DrawCrosshairIS = false --Draw the crosshair in ironsights?
SWEP.PrintName				= "Remington ACR"		-- Weapon name (Shown on HUD)
...
SWEP.ViewModelBoneMods = {
	...
	["A_Optic"] = { scale = Vector(0.85, 0.85, 0.85), pos = Vector(0, 0, 0), angle = Angle(0, 0, 0) },
...
SWEP.WElements = {
	...
	["flashlight"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/w_laser_sec.mdl", bone = "", rel = "", pos = Vector(22.2, 9.2, 3.9), angle = Angle(0, 0, -90), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = false, active = false },
...
function SWEP:Think2(...)
    ...
    if self.Owner:KeyPressed(IN_ZOOM) and self:GetIronSights() then 
        if self.CurrentSightState == 1 then 
			...
			self.IronSightsPos = Vector(-3.56, -4.193, 1.4)
			self.IronSightsAng = Vector(0.2, 0, 0)
			self.IronSightsPos_Kobra = Vector(-3.566, -4.393, 1.49)
			self.IronSightsAng_Kobra = Vector(0, 0, 0)
			self.IronSightsPos_EOTech = Vector(-3.565, -4.393, 1.3)
			self.IronSightsAng_EOTech = Vector(0, 0, 0)
			self.IronSightsPos_RDS = Vector(-3.566, -5.393, 1.28)
			self.IronSightsAng_RDS = Vector(0, 0, 0)
			self.IronSightsPos_2XRDS = Vector(-3.568, -6.2, 1.28)
			self.IronSightsAng_2XRDS = Vector(0, 0, 0)
			self.IronSightsPos_C79 = Vector(-3.57, -5, 0.92)
			self.IronSightsAng_C79 = Vector(0, 0, 0)
			self.IronSightsPos_PO4X = Vector(-3.504, -6, 1.385)
