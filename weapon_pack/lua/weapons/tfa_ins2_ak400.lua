...
SWEP.Purpose				= "A prototype modernized AK-74. Chambered in 5.45x39." --Purpose Tooltip
...
SWEP.PrintName				= "AK-400"		-- Weapon name (Shown on HUD)
...
--[[WEAPON HANDLING]]--
SWEP.Primary.Sound = Sound("TFA_INS2.AK400.Semi") -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("TFA_INS2.AK400.Semi_Silenced") -- This is the sound of the weapon, when silenced.
SWEP.Primary.GLSound = Sound("TFA_INS2_GP30.1")

SWEP.Primary.LoopSound = Sound("TFA_INS2.AK400.FireLoop") -- Looped fire sound, unsilenced
SWEP.Primary.LoopSoundTail = Sound("TFA_INS2.AK400.FireLoop_Tail") -- Loop end/tail sound, unsilenced
SWEP.Primary.LoopSoundSilenced = Sound("TFA_INS2.AK400.FireLoop_Silenced") -- Looped fire sound, silenced
SWEP.Primary.LoopSoundTailSilenced = Sound("TFA_INS2.AK400.FireLoop_Silenced_Tail") -- Loop end/tail sound, silenced
...
SWEP.Primary.Velocity = 1000 -- m/s

SWEP.FiresUnderwater = false

SWEP.SafetyAng = Vector(-9, 35, -10)
--Miscelaneous Sounds
...
--[[VIEWMODEL]]--
SWEP.ViewModel			= "models/weapons/tfa_ins2/c_ak400.mdl" --Viewmodel path
SWEP.ViewModelFOV			= 70 -- This controls how big the viewmodel looks.  Less is more.
SWEP.ViewModelFlip			= false		-- Set this to true for CSS models, or false for everything else (with a righthanded viewmodel.)
SWEP.UseHands = true --Use gmod c_arms system.
SWEP.VMPos = Vector(0.1, 2, -0.) --The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
...
--}
--[[WORLDMODEL]]--
SWEP.WorldModel			= "models/weapons/tfa_ins2/w_ak400.mdl" -- Weapon world model path
...
SWEP.IronSightsPos = Vector(-3.4675, 0, 1.388)
SWEP.IronSightsAng = Vector(-0.004, 0.005, 0)

SWEP.IronSightsPos_GL = Vector(-1.4, -4, 1.89)
...
SWEP.IronSightsPos_NVPoint = Vector(-5.8, 7, 0.5)
...
SWEP.StatusLengthOverride = {
	...
	["foregrip_reload"] = 72 / 30,
	["foregrip_reload_empty"] = 76 / 30,
	["gl_reload"] = 72 / 30,
	["gl_reload_empty"] = 76 / 30,
...
function SWEP:UpdateBeltBG(vm, addAmmo)
	local limit = vm:GetBodygroupCount(vm:FindBodygroupByName("Belt")) - 1
	...
	self.Bodygroups_V["Belt"] = m_C(target, 0, limit)
end

function SWEP:UpdateBeltBGOffScreen(vm)
	self:UpdateBeltBG(vm, true)
end

SWEP.EventTable = {
	["base_reload"] = {
		{ time = 0, type = "lua", value = SWEP.UpdateBeltBG },
		{ time = 42 / 30, type = "lua", value = SWEP.UpdateBeltBGOffScreen },
	},
	["base_reload_empty"] = {
		{ time = 0, type = "lua", value = SWEP.UpdateBeltBG },
		{ time = 46 / 30, type = "lua", value = SWEP.UpdateBeltBGOffScreen },
	},
	["foregrip_reload"] = {
		{ time = 0, type = "lua", value = SWEP.UpdateBeltBG },
		{ time = 42 / 30, type = "lua", value = SWEP.UpdateBeltBGOffScreen },
...
SWEP.ViewModelBoneMods = {
	["A_LaserFlashlight"] = { scale = Vector(1, 1, 1), pos = Vector(0, -2, 0), angle = Angle(0, 0, 0) },
	["A_Foregrip"] = { scale = Vector(.5, .5, .5), pos = Vector(0, 0.5, 0), angle = Angle(0, 0, 0) },
}

SWEP.WorldModelBoneMods = {
	["ATTACH_Laser"] = { scale = Vector(1, 1, 1), pos = Vector(0, 0, 0), angle = Angle(90, 0, 0) },
	["ATTACH_Muzzle"] = { scale = Vector(1, 1, 1), pos = Vector(0, 2.5, 0), angle = Angle(0, 0, 0) },
	["ATTACH_Foregrip"] = { scale = Vector(.75, .75, .75), pos = Vector(0, 0.75, 0), angle = Angle(0, 0, 0) },
}

--[[
	Selectable reticle code.
	If you're gonna use it in your weapon you must credit me (YuRaNnNzZZ) for that.
]]--
...
function SWEP:CycleReticle()
	...
	local activeelem = self:GetStat("INS2_SightVElement")
	if not activeelem then return end
...
SWEP.VElements = {
	["standard_barrel"] = { type = "Bool", active = true},
	...
	["foregrip"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_foregrip_ins.mdl", bone = "", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = color_white, surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = false },
	...
	["flashlight"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_laser_anpeq15_band.mdl", bone = "", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = false },
...
SWEP.WElements = {
	...
	["sight_1p87"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/w_1p87.mdl", bone = "", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = false },
...
SWEP.AttachmentExclusions = {
	["ins2_fg_gp25"] = { "ins2_ub_laser", "ins2_ub_flashlight1" },
	["ins2_ub_laser"] = { "ins2_fg_gp25" },
	["ins2_ub_flashlight1"] = { "ins2_fg_gp25" },
}
...
function SWEP:Think2(...)
	self.Bodygroups_V["Muzzlebrake"] = self:GetStat("VElements.standard_barrel.active") and 0 or 1
	self.Bodygroups_W["Muzzlebrake"] = self:GetStat("WElements.standard_barrel.active") and 0 or 1
...
function SWEP:Think2(...)
    ...
    if self.Owner:KeyPressed(IN_ZOOM) and self:GetIronSights() then 
        if self.CurrentSightState == 1 then 
			...
			self.IronSightsPos_1P87 = Vector(-3.4685, -4, -0.178)
			self.IronSightsAng_1P87 = Vector(0, 0, 0)
			self.IronSightsPos = Vector(-3.4675, 0, 1.388)
			self.IronSightsAng = Vector(-0.004, 0.005, 0)
			self.IronSightsPos_Kobra = Vector(-3.497, -4, 0.4385)
			self.IronSightsAng_Kobra = Vector(0, 0, 0)
			self.IronSightsPos_EOTech = Vector(-3.4965, -4, 0.45)
			self.IronSightsAng_EOTech = Vector(0, 0, 0)
			self.IronSightsPos_RDS = Vector(-3.4965, -4, 0.45)
			self.IronSightsAng_RDS = Vector(0, 0, 0)
			self.IronSightsPos_2XRDS = Vector(-3.4926, -4, 0.4465)
			self.IronSightsAng_2XRDS = Vector(0, 0, 0)
			self.IronSightsPos_C79 = Vector(-3.495, -4, 0.3181)
