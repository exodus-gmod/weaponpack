...
SWEP.Category        =
"EXR - SMGs"                                             --The category.  Please, just choose something generic or something I've already done if you plan on only doing like one swep..
...
SWEP.Purpose         = "A highly effective PDW chambered in 4.6 rounds. Comes with a high capacity magazine."                                --Purpose Tooltip
...
--[[WEAPON HANDLING]]                                    --
SWEP.Primary.Sound = Sound("TFA_INS2.MP7.1")             -- This is the sound of the weapon, when you shoot.
SWEP.Primary.SilencedSound = Sound("TFA_INS2.MP7.2")     -- This is the sound of the weapon, when silenced.
...
SWEP.Primary.Ammo =
"smg1"                                        -- What kind of ammo.  Options, besides custom, include pistol, 357, smg1, ar2, buckshot, slam, SniperPenetratedRound, and AirboatGun.
SWEP.Primary.AmmoConsumption = 1              --Ammo consumed per shot
--Pistol, buckshot, and slam like to ricochet. Use AirboatGun for a light metal peircing shotgun pellets
SWEP.DisableChambering = false                --Disable round-in-the-chamber
--Recoil Related
SWEP.Primary.KickUp = 0.1375                  -- This is the maximum upwards recoil (rise)
...
--[[VIEWMODEL]]                                         --
SWEP.ViewModel      = "models/weapons/tfa_ins2/c_mp7.mdl" --Viewmodel path
...
--[[WORLDMODEL]]                                        --
SWEP.WorldModel                = "models/weapons/tfa_ins2/w_mp7.mdl" -- Weapon world model path
...
SWEP.Idle_Mode = TFA.Enum
.IDLE_BOTH                                 --TFA.Enum.IDLE_DISABLED = no idle, TFA.Enum.IDLE_LUA = lua idle, TFA.Enum.IDLE_ANI = mdl idle, TFA.Enum.IDLE_BOTH = TFA.Enum.IDLE_ANI + TFA.Enum.IDLE_LUA
...
SWEP.WorldModelBoneMods = {
	...
	["ATTACH_Laser"] = { scale = Vector(1, 1, 1), pos = Vector(-0.25, 2, -0.15), angle = Angle(0, 180, -90) },
}
...
SWEP.VElements = {
	...
	["sight_kobra_lens"] = (TFA.INS2 and TFA.INS2.GetHoloSightReticle) and TFA.INS2.GetHoloSightReticle("sight_kobra") or
	nil,
	...
	["sight_eotech_lens"] = (TFA.INS2 and TFA.INS2.GetHoloSightReticle) and TFA.INS2.GetHoloSightReticle("sight_eotech") or
	nil,
	...
	["laser"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_laser_rail.mdl", bone = "A_Underbarrel", rel = "", pos = Vector(0, 0, 0), angle = Angle(0, 0, 0), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, bonemerge = true, active = false },
...
function SWEP:Think2(...)
	...
	if self.Owner:KeyPressed(IN_ZOOM) and self:GetIronSights() then
		if self.CurrentSightState == 1 then
			...
			self.IronSightsPos         = Vector(-2.821, 0, 0.775)
			self.IronSightsAng         = Vector(0.032, 0, 0)
			self.IronSightsPos_Kobra   = Vector(-2.823, -2, 0.39)
			self.IronSightsAng_Kobra   = Vector(0, 0, 0)
			self.IronSightsPos_EOTech  = Vector(-2.823, -2, 0.38)
			self.IronSightsAng_EOTech  = Vector(0, 0, 0)
			self.IronSightsPos_RDS     = Vector(-2.822, -2, 0.38)
			self.IronSightsAng_RDS     = Vector(0, 0.03, 0)
			self.IronSightsPos_2XRDS   = Vector(-2.82, -6, 0.38)
			self.IronSightsAng_2XRDS   = Vector(0, 0, 0)
			self.IronSightsPos_C79     = Vector(-2.8219, -6, 0.1)
			self.IronSightsAng_C79     = Vector(0, 0, 0)
			self.IronSightsPos_PO4X    = Vector(-2.77, -6, 0.47)
			...
			self.IronSightsPos_PO4X = self.IronSightsPos_NVPoint
